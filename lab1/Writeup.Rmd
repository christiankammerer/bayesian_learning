...
p <- ggplot() +
  aes(x = (1:10000), y = n_means, color = "Sample SD") +
  geom_line() +
  xlab("Number of Draws") +
  ylab("Mean across the first i draws") +
...
hist(phis, main = expression(paste("Distribution of ", phi)), 
     xlab = expression(phi), freq = FALSE)
...
$$L(\lambda; y) = \prod^n_{i=1} \frac{\lambda^{k_i}e^{-\lambda}}{k_i!}=\frac{e^{-n\lambda}\cdot\lambda^{\sum^n_{i=1}k_i}}{\prod^n_{i=1}k_i!}\propto e^{-n\lambda}\cdot\lambda^{\sum^n_{i=1}k_i}$$
...
$$
p(\lambda|y,\sigma) \propto e^{7\lambda+\frac{-\lambda^2}{50}}
$$
